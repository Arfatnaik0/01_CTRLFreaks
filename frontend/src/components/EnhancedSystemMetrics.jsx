...
const EnhancedSystemMetrics = ({ stats = {} }) => {
  ...
  const fetchMLInsights = async () => {
    try {
      const [anomaliesRes, maintenanceRes] = await Promise.all([
      ...
      if (anomaliesData?.anomalies) {
        ...
        recentAnomalies = anomaliesData.anomalies.filter(
      ...
      setMlInsights({
        ...
        maintenanceAlerts: maintenanceData?.count || 0,
        energyEfficiency: 89, // Static for now, could be dynamic
        predictiveScore: 94
      });
    } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
      ...
      <div className="bg-gray-700 rounded-lg p-6">
        ...
        <h4 className="text-2xl font-bold text-white mb-1">{stats.total_devices || 33}</h4>
      ...
      <div className="bg-gray-700 rounded-lg p-6">
        ...
        <h4 className="text-2xl font-bold text-white mb-1">{stats.total_energy_consumption?.toFixed(1) || '127.3'} kWh</h4>
